...
void pool_init(Pool *p, size_t item_size) {
	...
	p->chunks = (uint8_t**)malloc(sizeof(void*) * POOL_INITIAL_CHUNK_COUNT);
	for(size_t i = 0; i < POOL_INITIAL_CHUNK_COUNT; ++i)
		p->chunks[i] = (uint8_t*)malloc(item_size << 6);
}
...
void* pool_alloc(Pool *p) {
    if(p->occupied >= 64) {
        ...
        if(p->cur_chunk >= p->chunks_available) {
            ...
            p->chunks = (uint8_t**)realloc(p->chunks, sizeof(uint8_t*) * p->chunks_available);
            for(uint32_t i = p->cur_chunk; i < p->chunks_available; ++i)
                p->chunks[i] = (uint8_t*)malloc(p->item_size << 6);
        }
    }
